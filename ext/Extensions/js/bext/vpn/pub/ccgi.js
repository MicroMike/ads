...
function ccgi_ipc_handler(msg, resp_cb, sender){
    ...
    case 'fetch_rules':
        ...
        return !msg.no_resp;
    case 'check_needs_premium':
        msg.data = {};
        if (!RMT || !RMT.be_premium)
            return msg.data.err = 'no be_premium';
        E.sp.spawn(etask({name: 'check_needs_premium', cancel: true}, [
        function(){
            return !RMT.be_premium.is_active() &&
                RMT.be_premium.get_force_premium_rule(msg.opt.root_url);
        }, function(res){
            return msg.data.result = res;
        }, function catch$(err){ return msg.data.err = ''+err;
        }, function finally$(){
            if (!msg.no_resp)
                ccgi_resp(msg, resp_cb);
    ...
    case 'trial_subscribe':
        try { E.sp.spawn(RMT.be_info.trial_subscribe(msg.data)); }
        catch(e){}
        break;
