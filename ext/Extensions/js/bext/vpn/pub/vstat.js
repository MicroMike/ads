...
// XXX arik: copy from util/escape.js qs()
...
// XXX colin: remove all try catch and use window.onerror to send a perr
try {
if (is_node)
...
define2([], function(){
try {
// XXX denis: copy from util/user_agent.js
...
// XXX arik: rename customer to hola_vstat (or take from page)
...
E.perr_raw = function(opt){
    ...
    qs.id = opt.id;
    // XXX arik: need os detection
    qs.customer = opt.customer;
    // XXX arik: need tag id/date/version
    qs.zone = opt.zone;
...
// XXX arik: copy from util/util.js
...
// XXX arik/nir: code copied from pkg/util/events.js because this
// file is used by customers without require. need to find a
// solution to avoid copy of the file (also need to fix coding)
...
html5.Player = function(opt){
    ...
    // XXX arik: add progress timeupdate events
    this.cb = {};
    ...
    this.interval_200ms = setInterval(function(){
        ...
        if (isFinite(view.pos)&&isFinite(pos))
        {
            if (this.view.every==undefined)
                this.view.every = pos;
            var d = pos-view.pos;
            if (d>0 && d<2) // ignore backward seek/forward seek
                view.sec+=d;
        }
        view.pos = pos;
...
Stat.prototype.push = function(e, opt){
    ...
    try {
        ...
        case 'seeking':
            ...
            break;
        case 'seeked':
            if (!this.seeking_ts) // XXX arik: may happen if we attach late
...
var detect_video = function(){
    ...
    for (var i = 0; i < a.length; i++)
    {
        ...
        // XXX arik: handle replay after ended
...
E.init = function(){
    ...
    window.addEventListener('beforeunload', function(){
        E.perr({id: 'unload'}); });
    // XXX michaelg : support <ie11 : github.com/stubar/jquery.domChanged
